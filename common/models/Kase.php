...
class Kase extends MyActiveRecord
{
	...
	public function rules()
	{
		return [
			...
			[['name', 'info', 'why_closed', 'closed_note', 'web_referral', 'web_keyword', 'emails'], 'trim'],
			[['name', 'owner_id', 'is_priority', 'language', 'is_b2b', 'how_contacted', 'how_found'], 'required', 'message'=>'Field is required'],
			[['why_closed', 'closed_note'], 'required', 'on'=>'cases_close'],
			[['company_id', 'ref', 'campaign_id'], 'integer', 'min'=>0],
			[['ref'], 'default', 'value'=>0],
		];
	}
