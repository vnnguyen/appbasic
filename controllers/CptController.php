...
//////////////////dntt////////////////////
    public function actionPdf($id)
    {
        ...
        Yii::$app->language = 'vi';
        $ltt = AtLtt::find()->where('id = '.$id)->one();
        if ($ltt == null) {
        ...
        if ($ltt->mtt_ids != null) {
            ...
            ->where(['id' => explode(',', $ltt->mtt_ids)])->all();
            $t_amount = [];
            $money_group = [];
            $moneys = [];
            ...
            foreach ($moneys as $k => $v) {
                $t_amount[] = $v.'-'.$k;
                        ...
                        //end
            $content = $this->renderPartial('content_pdf',['model' => $ltt, 'mtts' =>$dntts, 't_amount' => $t_amount]);
        }
        ...
        $pdf = new Pdf([
            ...
            .main-content h1{ font-size: 18px; color: #ed3691; font-weight: bold; text-align: center; line-height: 30px}
            .pdf-list, .info, .text-money {padding: 0 10px; }
            .text-money p { padding: 0px 25px; text-transform: capitalize; font-size: 12px; line-height: 12px}
            .text-money div{font-size: 12px; margin-bottom: 8px}
            .info p {font-size: 13px; line-height: 14px}
            .text-money p span{text-transform: uppercase;}
            .pdf-list .program table { margin-top: 10px; border: 1px solid #bfbfbf}
            ...
            .pdf-list .program table , .pdf-list .program table th{text-align:center; font-size: 13px}

            .bottom-div {padding-left: 20px; font-size: 13px;}
            .bottom-div table {font-size: 13px}
            .bottom-div table th{ text-align: left; width: 155px; padding-bottom: 100px}
            .bottom-div .dntt-date {padding-right: 15px}
            ',
    ...
    public function actionCpt_input()
    {
        if (isset($_POST['save'])) {
            $payDates = $_POST['payDate'];
            $USER_ID = 1;
            foreach ($payDates as $k => $payDate) {
                if ($payDate != '' && $_POST['amount'][$k] != '' && $_POST['currency'][$k] && $_POST['content'][$k]) {
                    $cpt = new Cpt();
                    $cpt->created_by = $USER_ID;
                    $cpt->created_at = date('Y/m/d H:i:s', strtotime('now'));
                    $cpt->tour_id = 0;
                    $arr_day = explode('/', $payDate);
                    $cpt->dvtour_day = date('Y/m/d', strtotime($arr_day[2].'/'.$arr_day[1].'/'.$arr_day[0]));
                    $cpt->dvtour_name = $_POST['content'][$k];
                    $cpt->qty = 1;
                    $cpt->price = $_POST['amount'][$k];
                    $cpt->unitc = $_POST['currency'][$k];

                    //
                    $cpt->updated_at = date('Y/m/d H:i:s', strtotime('now'));
                    $cpt->updated_by = $USER_ID;
                    $cpt->dv_id = 0;
                    $cpt->venue_id = 0;
                    $cpt->cp_id = 0;
                    $cpt->oppr = '';
                    $cpt->user_id = $cpt->updated_by;
                    $cpt->start = '00:00:00';
                    $cpt->length = 0;
                    $cpt->number = '';
                    $cpt->by_company_id = 0;
                    $cpt->via_company_id = 0;
                    $cpt->unit = '';
                    $cpt->booker = '';
                    $cpt->payer = '';
                    $cpt->approved = date('Y/m/d', strtotime('0'));
                    $cpt->approved_by = 0;
                    $cpt->paid = date('Y/m/d', strtotime('0'));
                    $cpt->confirmed = date('Y/m/d', strtotime('0'));
                    $cpt->due = date('Y/m/d', strtotime('0'));
                    $cpt->vat_by = 0;
                    $cpt->xacnhan_date = date('Y/m/d', strtotime('0'));
                    $cpt->xacnhan_by = 0;
                    $cpt->duyet_date = date('Y/m/d H:i:s', strtotime('0'));
                    $cpt->duyet_by = 0;
                    $cpt->c8 = $cpt->c7 = $cpt->c6 = $cpt->c5 = $cpt->c4 = $cpt->c3 = $cpt->c2 = $cpt->c1 = '';
                    $cpt->xd = '0.0';
                    if ($cpt->validate() && $cpt->save()) {
                        ...
                        $theMtt->cpt_id = $cpt->dvtour_id;
                        $theMtt->status = 'draft';
                        $theMtt->paid_in_full = 0;
                        $theMtt->currency = $cpt->unitc;
                        $theMtt->xrate = $USER_ID;
                        $theMtt->rem_amount = $cpt->qty * $cpt->price;
                        $theMtt->amount = $cpt->qty * $cpt->price;
                        $theMtt->ltt_id = 0;
                        if ($theMtt->save()) {
                            Yii::$app->getSession()->setFlash('success', 'Saved mtt success');
                        } else {
                            Yii::$app->getSession()->setFlash('err', 'Mtt saved unsuccess');
                        }
                    } else {
                        Yii::$app->getSession()->setFlash('err', 'Cpt saved unsuccess');
                    }
                }
            }
        }
        return $this->render('cpt_input', []);
    }
                ...
                if ($ltt->save()) {
                    ...
                    if ($mtts != null) {
                        foreach ($mtts as $mtt) {
                            ...
                            ->where('cpt_id ='.$mtt->cpt_id.' AND status = "draft" AND ltt_id = 0')->one();
                            if ($mtt_new != null) {
                                $mtt->status = 'off';
                                //
                                $dntts = AtMtt::find()->where('ltt_id != 0 AND id != '.$mtt->id.' AND cpt_id = '.$mtt->cpt_id)->all();
                                $r_amount = 0;
                                $total_m = 0;
                                if ($dntts != null) {
                                    foreach ($dntts as $dntt) {
                                    ...
                                    $mtt_new->rem_amount = $mtt_new['cpt']['qty'] * $mtt_new['cpt']['price'] - $total_m;
                                } else {
                                    $mtt_new->rem_amount = $mtt_new['cpt']['qty'] * $mtt_new['cpt']['price'];
                                }
                                $mtt_new->save();
                            }
                ...
                if ($ltt) {
                    if (in_array($mtt->id, explode(',', $ltt->mtt_ids))) {
                        ...
                        if ($ltt->save()) {
                            ...
                            if ($mtt->delete()) {
                                echo json_encode(['m' => $t_amount, 'ltt' => ArrayHelper::toArray($ltt)]);
                            } else {
    ...
    public function actionCreate_ltt()
    {
        if (Yii::$app->request->isAjax) {
            ...
            $arr_ids = array_diff(explode(',', $_POST['ids']), ['']);
            $USER_ID = 1;
            $dntts = AtMtt::find()
            ->where(['id' => $arr_ids, 'created_by' => 1, 'status' => 'draft', 'ltt_id' => 0])->all();
            if ($dntts != null) {
                $ids = [];
                //phan loai tien
                $t_amount = [];
                ...
                if ($ltt->save()) {
                    ...
                    ->where(['cpt_id' => $cpt_ids])
                    ->andWhere('status != "deleted"')
                    ->asArray()
                    ->all();
                    if ($new_mtts == null) {
                        return json_encode(['error' => 'new mtt null']);
                    }
                    $arr_status = [];
                    foreach ($new_mtts as $new_mtt) {
                        if ($new_mtt['ltt_id'] != 0
                            && isset($new_mtt['ltt'])
                            && $new_mtt['ltt']['status'] == 'request')
                        {
                            $t_amount = $new_mtt['cpt']['qty'] * $new_mtt['cpt']['price'];
                            $cp_id = $new_mtt['cpt_id'];
                            $ltt_status = $new_mtt['ltt']['status'];
                            if (isset($arr_status[$cp_id][$ltt_status])) {
                                $arr_status[$cp_id][$ltt_status]['amount'] += $new_mtt['amount'];
                                $arr_status[$cp_id][$ltt_status]['percent'] = number_format($arr_status[$cp_id][$ltt_status]['amount'] / $t_amount * 100, 1);
                            } else {
                                $arr_status[$cp_id][$ltt_status]['status'] = $new_mtt['ltt']['status'];
                                $arr_status[$cp_id][$ltt_status]['amount'] = $new_mtt['amount'];
                                $arr_status[$cp_id][$ltt_status]['percent'] = number_format($new_mtt['amount'] / $t_amount * 100,1);
                            }
                        }
                    }
                    echo json_encode(ArrayHelper::toArray($arr_status));
                } else {
    ...
    public function actionEdit_dntt()
    {
        if (Yii::$app->request->isAjax) {
            if ($_POST['mtt_id'] != '' && $_POST['amount'] != '') {
                ...
                if ($_POST['amount'] > $dntt->rem_amount || $_POST['amount'] <= 0) {
                    return json_encode(['err' => "this MTT (".$_POST['amount'].")".$dntt->currency." is false"]);
                }
    ...
    public function actionCpt_percen($ids = '', $percen = 0)
    {
        if (Yii::$app->request->isAjax) {
            ...
            if ($ids != '' && $percen > 0) {
                ...
                if ($mtts != null) {
                    foreach ($mtts as $mtt) {
                        $amount = $percen * $mtt['cpt']['qty'] * $mtt['cpt']['price'] / 100;
                        if ($amount > $mtt->rem_amount) {
                            $mtt->amount = $mtt->rem_amount;
                            return json_encode(['err' => "MTT (".$mtt['cpt']['unit'].") is not save ".$amount." ".$mtt->currency]);
                        } else {
                            if ($amount < 0) {
                                return json_encode(['err' => "MTT (".$mtt['cpt']['unit'].") is not save ".$amount." ".$mtt->currency]);
                            }
                        ...
                        if (!$mtt->save()) {
                            return json_encode(['err' => 'mtt is not save']);
                        }
                    }
                    ...
                    echo json_encode(['err' => 'mtts is null']);
                }
            }
        }
    }
...
/////////////////////////////end dntt////////////////////////
    ...
    public function actionLink_dv($vat = '', $user = 'all', $tour = '', $dvtour = '', $search = '', $filter = '', $payer = '', $sign = '', $currency = '', $tt = '', $orderby = 'dvtour_day', $unit = '', $link = 'all', $limit = 25)
    {
        // if (MY_ID > 4 && !in_array(MY_ID, [1,2,3,4,28431,  11,   17,   16,  20787,29739, 30085, 25457])) {
        //     //throw new HttpException(403, 'Access denied.');
        // }
    ...
    //         ]);
    // }
    public function actionUnlink($cpt_id = 0)
    {
        if (Yii::$app->request->isAjax) {
            $cpt = Cpt::findOne($cpt_id);
            if ($cpt != null) {
                $USER_ID = 1;
                $cpt->dv_id = 0;
                $cpt->updated_at = date('Y/m/d H:i:s', strtotime('now'));
                $cpt->updated_by = $USER_ID;
                if ($cpt->save()) {
                    echo json_encode(['success' => ArrayHelper::toArray($cpt)]);
                } else {
                    echo json_encode(['err' => 'Unlink not Saved']);
                }
            } else {
                echo json_encode(['err' => 'cpt null']);
            }
        }
    }
    ...
    public function actionLink_dv_auto($link_auto = "all")
    {
        if ($link_auto != 'all') {
