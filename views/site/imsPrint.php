...
// var_dump();die();
$webRoot = Yii::$app->basePath; //Yii::getAlias('@webroot');
$baseUrl = Yii::$app->request->baseUrl;

require_once($webRoot.'/helpers/hxTextile.php');
    ...
    if (file_exists($dir)) {
        ...
        foreach ($photos as $pt) {
            ...
            echo "<li>
                    ...
                    <img class='img-devis' style='width: 150px' src='".$webRoot.DIR."upload/banner_date_tour_devis/$folderName/$pt'>
...
<html lang="fr">
    ...
    <body style="font-family:Calibri; font-size:9pt;">
        <div style="width:800px; margin:auto;">
            ...
            <p style="font-family:Calibri; font-size:11pt;">ĐÂY LÀ TEXT DÙNG CHO FILE MẪU DEVIS MỚI NHẤT CỦA AMICA (14 TEMPLATE WORD KHÁC NHAU)      
            </p>
            <div>
                ...
                <p>B2B tiếng ANH New click vào <a href="<?=DIR?>imsprint-b2b-en/<?=SEG2?>/<?=SEG3?>">ĐÂY</a></p>
                    ...
                    <? foreach ($temArr as $key => $value) : ?>
                        ...
                        <h2><?=$key ?></h2>
                          <? foreach ($value as $ki => $vi) :?>
                              <li>
                                    <p><?=$vi['name'] ?></p>
                                    <img class="img-devis" style="width: 70px" src='<?= $baseUrl.DIR ?>assets/img/thumb/<?=$vi['thumb'] ?>.JPG'>
                    ...
                    <ul>
                     <?
                                   // $dir = '/var/www/www.amica-travel.com/upload/image_devis_secretindochina/';
                                    $dir = $webRoot.'/upload/banner_2_devis/b2c';
                                    $imageFolders = glob($dir.'/*', GLOB_ONLYDIR);
                                    var_dump($imageFolders);exit();
                                       ...
                                       $photos = FileHelper::findFiles($value, ['recursive' => false, 'only' => ['*.jpg','*.png']]);
                                        foreach ($photos as $pt) :
                                            ...
                                            echo "<li>
                                                ...
                                                <img class='img-devis' style='width: 150px' src='".DIR."timthumb.php?src=".DIR."upload/banner_2_devis/".$pt."&w=150&h=60&zc=1'>
                ...
                <div>
                 <a href="#image-footer" class="fancybox topopup add-image-footer">Thay thế ảnh trong footer</a>
                            ...
                            <div id="popup_content"> <!--your content start-->
                                <div class="image-tableau"><!--Start Image-->
                                   ...
                                   <ul>
                                    <?
                                   // $dir = '/var/www/www.amica-travel.com/upload/image_devis_secretindochina/';
                                    $dir = 'upload/banner_2_devis/image_tableau';
                                    if (file_exists($dir)) {
            ...
            <div id='sale-detail'>
                ...
                <h2 id='devis-sale-name' style='margin: 0; padding: 0;font-family:Calibri; font-size: 11pt;color:black;'>Mlle. <?=$theProduct['createdBy']['fname'] . ' ' . $theProduct['createdBy']['lname'] ?></h2>
...
//$ngay_fr = array('Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche');
            ...
            <div id='devis-table-tarif'>
                ...
                <p style="font-family:Calibri; font-size:9pt;">Conditions tarifaires établies le <?= date('d-m-Y', strtotime($theProduct['created_at'])) ?></p>
...
<td style="border-bottom: 1px solid #B0A47A;font-family:Calibri; font-size:9pt; padding: 5pt 2mm;" class="a-href">' . (filter_var(trim($line[3]), FILTER_VALIDATE_URL) == true ? Html::a( trim($line[3]), trim($line[3]), ['class' => 'profile-link']) : str_replace('http://','',trim($line[3]))) . '</td></tr>';
            ...
            <div id='devis-promotion' style='text-align: center;'>
                    ...
                    <span style='font-weight:bold; color: #BD3920; font-size: 14pt; font-family:Calibri;'>Promotion: </span><span style='font-weight:bold; color: #BD3920; font-size: 11pt; font-family:Calibri;'><?= $txt->textileThis($theProduct['promo']) ?></span>
                    <? else: echo "";
                    endif;
                    ?>
            </div>
                                   ...
                                   // $dir = '/var/www/www.amica-travel.com/upload/image_devis_secretindochina/';
                                    ...
                                    if (file_exists($dir)) {
                                        ...
                                        foreach ($photos as $pt) :
                                            ...
                                            echo "<li>
                                                <p>".str_replace(['-', '.png', '.jpg'], [' ', '', ''], $pt)."</p>
                                                <img class='img-devis' style='width: 150px' src='".DIR."timthumb.php?src=".DIR."upload/img-devis/footer/".$pt."&w=150&h=60&zc=1'>
                                                <input type='checkbox' value='".$pt."'/>
                                                </li>";
...
<style>
    ...
    .jour .remove{
        ...
        width: 40px;
        display: none;
        background: url('<?=DIR?>assets/img/xx.png') center center no-repeat;
...
div.loader {
    background: url("<?=DIR?>assets/img/loading.gif") no-repeat scroll 0 0 transparent;
    height: 32px;
    width: 32px;
    display: none;
    z-index: 9999;
...
div.close {
    background: url("<?=DIR?>assets/img/closebox.png") no-repeat scroll -4px -3px transparent;
    cursor: pointer;
    height: 30px;
    position: absolute;
    right: -27px;
      ...
      //$('p em').parent().css('color','rgb(155, 46, 119)');
        ...
        $('.add-image-jours').click(function() {
         ...
         $('.add-image-tableau').click(function() {
          ...
          $('.add-image-footer').click(function() {
            $('#popup_content .test').hide();
            $('#popup_content .add-image-footer').show();
        });
    });
    ...
    $('#download-word,#downloadword').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#loading').hide();
                var win = window.open('<?php echo DIR . 'upload/output/devis-ims/' ?>' + file_name + '.docx', '_blank');
                win.focus();
            },
            async: false
