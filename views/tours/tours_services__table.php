...
<table id="okle" class="table table-condensed">
	<thead>
	<tr>
		<th width="17%"><?= Yii::t('tours_services__table', 'Nội dung'); ?></th>
		<th width="15%"><?= Yii::t('tours_services__table', 'Đối tác / Cung cấp'); ?></th>
		<th width="3%"><?= Yii::t('tours_services__table', 'Số'); ?></th>
		<th width="7%"><?= Yii::t('tours_services__table', 'Đvị'); ?></th>
		<th width="10%"><?= Yii::t('tours_services__table', 'Giá tiền'); ?></th>
		<? if (!in_array(myID, array(3404, 5805, 14029, 14030, 15007))) { ?>
		<th width="11%"><?= Yii::t('tours_services__table', 'Thành VND'); ?></th>
		<? } ?>
		<th width="12%"><?= Yii::t('tours_services__table', 'Admin/Resv'); ?>?</th>
		<th width="12%"><?= Yii::t('tours_services__table', 'Ai thanh toán'); ?></th>
		<th width="13%"><?= Yii::t('tours_services__table', 'Cho kế toán'); ?></th>
	</tr>
	</thead>
	<tbody>
			...
			<form id="form-dv" class="form-horizontal" method="post" action="">
				<fieldset>
					...
					<div class="control-group">
						<label class="control-label" for=""><?= Yii::t('tours_services__table', 'Day in tour'); ?></label>
						<div class="controls">
...
foreach ($dayIdList as $di) {
	foreach ($ctDays as $ng) {
		if ($ng['id'] == $di) {
					...
					<div class="control-group">
						<label class="control-label" for=""><?= Yii::t('tours_services__table', 'Name of service'); ?></label>
						<div class="controls">
							<input class="blank span3" type="text" name="dvtour_name" value="">
							<?= Yii::t('tours_services__table', 'Provider/Venue'); ?>
							<input class="blank span3" type="text" name="oppr" value="">
							<?= Yii::t('tours_services__table', 'Admin'); ?>
							<select class="span3" name="adminby">
								<option value="hn"><?= Yii::t('tours_services__table', 'Ha Noi'); ?></option>
								<option value="vt"><?= Yii::t('tours_services__table', 'Vientiane'); ?></option>
								<option value="sr"><?= Yii::t('tours_services__table', 'Siem Reap'); ?></option>
								<option value="nt"><?= Yii::t('tours_services__table', 'Nha Trang'); ?></option>
							</select>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for=""><?= Yii::t('tours_services__table', 'Venue/location'); ?></label>
						<div class="controls">
							<select class="span5 select2" name="venue_id">
								<option value="0">- <?= Yii::t('tours_services__table', 'Hotel, sightseeing'); ?>... -</option>
								<? foreach ($allVenues as $vn) {?>
								<option value="<?=$vn['id']?>"><?=$vn['name']?></option>
								<? } ?>
							</select>
							<?= Yii::t('tours_services__table', 'Time'); ?>
							<input class="span1" type="text" name="start" value="" maxlength="4" />
							<?= Yii::t('tours_services__table', 'Number'); ?>
							<input class="span2" type="text" name="number" value="" maxlength="30" />
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for=""><?= Yii::t('tours_services__table', 'Nhà cung cấp'); ?></label>
						<div class="controls">
							<select class="span4 select2" name="by_company_id">
								<option value="0">- <?= Yii::t('tours_services__table', 'Tàu, xe, dịch vụ gốc'); ?>... -</option>
								<? foreach ($allCompanies as $c) { ?>
								<option value="<?=$c['id']?>"><?=$c['name']?></option>
								<? } ?>
							</select>
							<?= Yii::t('tours_services__table', 'Đại lý'); ?>
							<select class="span4 select2" name="via_company_id">
								<option value="0">- <?= Yii::t('tours_services__table', 'Vé tàu, vé máy bay, package'); ?>... -</option>
								<? foreach ($allCompanies as $c) { ?>
								<option value="<?=$c['id']?>"><?=$c['name']?></option>
								<? } ?>
							</select>
					...
					<div class="control-group">
						<label class="control-label" for=""><?= Yii::t('tours_services__table', 'Quantity'); ?></label>
						<div class="controls">
							<input class="blank span1 ta-r" type="text" name="qty" value="">
							<?= Yii::t('tours_services__table', 'Unit'); ?>
							<input class="blank span2" type="text" name="unit" value="">
							<?= Yii::t('tours_services__table', 'Unit price'); ?>
					...
					<div class="control-group">
						<label class="control-label" for=""><?= Yii::t('tours_services__table', 'Prebooking needed'); ?>?</label>
						<div class="controls">
							<select class="span2" name="prebooking">
								<option value="yes">Yes / Có</option>
								<option value="no">No / Không</option>
							</select>
							<?= Yii::t('tours_services__table', 'Who pays'); ?>
							...
							<?= Yii::t('tours_services__table', 'Booking status'); ?>
							<select class="span2" name="status">
								<option value="n">Not OK</option>
								<option value="k">OK</option>
							</select>
							<?= Yii::t('tours_services__table', 'Payment due'); ?>
							<input type="text" tabindex="-1" class="blank span2 datepicker" name="due" value="" />
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for=""><?= Yii::t('tours_services__table', 'Note (optional)'); ?></label>
						<div class="controls">
					...
					<div class="form-actions">
						<a id="form-dv-submit" class="btn btn-primary" href="#"><?= Yii::t('tours_services__table', 'Save changes'); ?></a>
						<a id="form-dv-copy" class="btn btn-info" href="#"><?= Yii::t('tours_services__table', 'Save as copy'); ?></a>
						<a id="form-dv-close" href="#"><?= Yii::t('tours_services__table', 'Cancel'); ?></a>
					</div>
				</fieldset>
	...
	<tr id="day<?=$k?>" class="info">
		...
		<td colspan="<?=!in_array(myID, array(3404, 5805, 14029, 14030, 15007)) ? 7 : 6?>">
			<? if ($v == '') { ?>
			<?= Yii::t('tours_services__table', 'Ngày này không nằm trong chương trình tour chính thức'); ?>
			<? } else { ?>
			<? echo anchor('#tours/ngaytour/'.seg3, $v['name'].' ('.$v['meals'].')', 'class="fw-b" title="'.str_replace('"', '`', $v['body']).'"');?>
			<? } ?>
		</td>
		<td><?=myID == $theTour['op'] || in_array(myID, $tourOperatorIds) ? '<a class="dvt-c" href="#xdvtour-create" day="'.$k.'">+ '.Yii::t('tours_services__table', 'Add service').'</a>' : ''?></td>
	</tr>
	<? foreach ($tgx as $tg) {  if ($getFilter == '' && $tg['day'] == $k) { ?>
	<tr>
		<td><span title="<?= Yii::t('tours_services__table', 'Điều hành đánh dấu đã đặt xong'); ?>" class="dvtour-ok s-status xacnhan">OK</span> <?=anchor('users/lichguide/'.$tg['user_id'].'?month='.substr($tg['day'], 0, 7), 'Tour guide')?></td>
	...
	<tr id="dvtour-<?=$s['dvtour_id']?>">
		<td class="show-on-hover">
			...
			?>
			<span title="<?= Yii::t('tours_services__table', 'Điều hành đánh dấu đã đặt xong'); ?>" class="dvtour-ok s-status <?=$s['status'] == 'k' ? 'xacnhan' : ''?>" rel="<?=$s['dvtour_id']?>">OK</span>
			<? if ($s['start'] != '00:00:00') { ?><span style="background:#069; padding:0 2px; color:#ffc;"><?=substr($s['start'], 0, 5)?></span><? } ?>
			<a title="<?= Yii::t('tours_services__table', 'Sửa dịch vụ tour'); ?> (ID: <?=$s['dvtour_id']?>)" rel="<?=$s['dvtour_id']?>" class="dvt-u" href="#cdvtour-update"><?=$s['dvtour_name']?></a>
			<a title="<?= Yii::t('tours_services__table', 'Thêm / Xem các ghi chú'); ?>" class="<?=$hasComments ? 'hasComments' : ' muted shown-on-hover'?> td-n" href="<?=DIR?>tours/mm/<?=$theTour['id']?>/<?=$s['dvtour_id']?>">+<?= Yii::t('tours_services__table', 'note'); ?></a>
		</td>
		<td><?
		if ($s['venue_id'] != 0) {
			...
			echo anchor(uris.'?filter=hn-'.md5($s['oppr']), $s['oppr']);
		}
		?>
		</td>
		<td class="ta-r"><?=trim(trim($s['qty'], '0'), '.')?></td>
		<td><?=$s['unit']?></td>
		<td class="ta-r">
			<? if ($s['plusminus'] == 'minus') echo '-'; ?><?=number_format($s['price'])?><span class="small quieter"><?=$s['unitc']?></span>
		</td>
		<?
		...
		<? if (!in_array(myID, array(3404, 5805, 14029, 14030, 15007))) { ?>
		<td class="ta-r <? if($s['approved_by'] !=0) {?>approved<? } ?>" title="<?=$s['unitc'] != 'VND' ? Yii::t('tours_services__table', 'Tỉ giá').': '.$xRates[$s['unitc']] : ''?>">
			<?
			...
			echo anchor('s-approve/'.$s['dvtour_id'], number_format($sub), 'title=" '.Yii::t('tours_services__table', 'Duyệt chấp nhận').': '.$approverNames.'" class="approve" rel="'.$s['dvtour_id'].'"');
		...
		?></td>
		<td><?=anchor(uris.'?filter=hp-'.md5($s['payer']), $s['payer'])?></td>
		<td>
			<span title="<?= Yii::t('tours_services__table', 'Kế toán đánh dấu đã thanh toán'); ?>" class="dvtour-tra pct<?=$s['thanhtoan_pct']?>" rel="<?=$s['dvtour_id']?>"><?= Yii::t('tours_services__table', 'TRẢ'); ?></span>
			<span title="<?= Yii::t('tours_services__table', 'Kế toán trưởng xác nhận'); ?>" class="dvtour-ktt <?=$s['xacnhan_by'] == 0 ? '' : 'xacnhan'?>" rel="<?=$s['dvtour_id']?>"><?= Yii::t('tours_services__table', 'KTT'); ?></span>
			<? if (myID == 2 || $appUser->hasRoles('ketoan')) { ?>
			<span title="<?= Yii::t('tours_services__table', 'Giám đốc duyệt lần cuối'); ?>" class="s-duyet <?=$s['duyet_by'] == 0 ? '' : 'xacnhan'?>" rel="<?=$s['dvtour_id']?>"><?= Yii::t('tours_services__table', 'GĐ'); ?></span>
			<? } ?>
			<span title="<?= Yii::t('tours_services__table', ''); ?>Kế toán đánh dấu VAT" class="s-vat <?=$s['vat_ok'] == '' ? '' : ($s['vat_ok'] == 'needed' ? 'pct50' : 'pct100')?>" rel="<?=$s['dvtour_id']?>">VAT</span>
