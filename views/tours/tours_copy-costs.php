...
?>
        ...
        <select class="form-control" name="dc">
            <option value="1">1 <?= Yii::t('tours_copy-costs', 'day');?></option>
            <? for ($i = 2; $i <= count($sourceProduct['days']); $i ++ ) { ?>
            <option value="<?= $i ?>"><?= $i ?> <?= Yii::t('tours_copy-costs', 'days');?></option>
            <? } ?>
        </select>
...
?>
        <button type="submit" class="btn btn-primary"><?= Yii::t('tours_copy-costs', 'Go');?></button>
<?
if (isset($sourceTour) && isset($destTour)) {
?>
        <a id="a-clear-all" href="#"><?= Yii::t('tours_copy-costs', 'Clear all');?></a>
        |
<?
} // if isset sourceTour
?>
        <a id="a-reset" href="<?= DIR ?>tours/copy-costs"><?= Yii::t('tours_copy-costs', 'Reset');?></a>
    </form>
</div>
<? if (isset($sourceTour) && isset($destTour)) { ?>
<div class="col-md-6">
...
?>
            <tr class="tr-day info">
                <td colspan="6">
                    <a class="pull-right a-sd" data-id="<?= $theDate ?>" href="#"><?= Yii::t('tours_copy-costs', 'Add to copy');?></a>
...
?>
            <tr class="tr-day info">
                <td colspan="6">
                    <a class="pull-right a-dd" data-id="<?= $theDate ?>" href="#"><?= Yii::t('tours_copy-costs', 'Add to copy');?></a>
                    <span class="label label-danger"><?= $cnt ?></span>
                    <strong><?= $theDate ?></strong>
                    <?= $day['name'] ?>
                    (<em><?= $day['meals'] ?></em>)
