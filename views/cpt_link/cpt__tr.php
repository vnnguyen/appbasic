...
?>
                <tr>
                    ...
                    <td><?= $cpt['payer'] ?></td>
                    <td><?= $cpt['due'] == '0000-00-00' ? '' : $cpt['due'] ?>
                        <?php if (isset($cpt['mtt']) && $cpt['mtt'] != null){ ?>
                            <div class="text-left progress-bar "><!-- stripes -->
                            <? 
                                $status_check = [];
                                $arr_status = [];
                                ...
                                if (isset($arr_status['check3'])) {?>
                                    <span class="<?= 'green'?>" title="<?= number_format($arr_status['check3']['amount'],2,'.',',')?>" data-amount="<?= $arr_status['check3']['amount']?>" style="width: <?= $arr_status['check3']['percent']?>%"><?= $arr_status['check3']['percent']?>%</span>
                                <? } ?>
                                <?
                                if (isset($arr_status['check2'])) {?>
                                    <span class="<?= 'red'?>" title="<?= number_format($arr_status['check2']['amount'],2,'.',',')?>" data-amount="<?= $arr_status['check2']['amount']?>" style="width: <?= $arr_status['check2']['percent']?>%"><?= $arr_status['check2']['percent']?>%</span>
                                <? } ?>
                                <?
                                if (isset($arr_status['check1'])) {?>
                                    <span class="<?= 'blue'?>" title="<?= number_format($arr_status['check1']['amount'],2,'.',',')?>" data-amount="<?= $arr_status['check1']['amount']?>" style="width: <?= $arr_status['check1']['percent']?>%"><?= $arr_status['check1']['percent']?>%</span>
                                <? } ?>
                                <?
                                if (isset($arr_status['request'])) {?>
                                    <span class="<?= 'request'?>" title="<?= number_format($arr_status['request']['amount'],2,'.',',')?>" data-amount="<?= $arr_status['request']['amount']?>" style="width: <?= $arr_status['request']['percent']?>%"><?= $arr_status['request']['percent']?>%</span>
                                <? } ?>
                            </div>
                            <?php }?>
                    ...
                    <td>
                            ...
                            <?php foreach ($cpt['rout'] as $rout): ?>
                                <?php foreach (explode(';', $rout['content']) as $dv_name): ?>
                                    <?php if (strtolower(trim($cpt['unit'])) == strtolower(trim($dv_name))): ?>
                                        <a class="sugggest_link" data-id="<?= $cpt['dvtour_id']?>" data-name="<?= $cpt['unit']?>" data-dv="<?= $rout['dv_id']?>" href="">Suggest link</a>
                                        <?php break;?>
                                    <?php endif ?>
                                <?php endforeach ?>
                            <?php endforeach ?>
                        ...
                        <?php if ($cpt['dv_id'] > 0): ?>
                            <div class="col-md-3 col-sm-4 <?= ($cpt['status_check'] == 'not ok')?'icon-link':'fa fa-check'?>  links" data-status="<?= $cpt['status_check']?>" data-id="<?= $cpt['dvtour_id']?>">
                            </div>
                        <?php endif ?>
                        <?
                            $total_amount_paid = 0;
